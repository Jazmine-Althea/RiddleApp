...
import { level12} from './hangmandata';
...
export default function Level12() {
  ...
  const correctWord = level12[currentIndex].answer;
  ...
  const updateStatus = (cl) => {
    ...
    if (status === 'win' && currentIndex === level12.length-1){
      setStatus('completed')
      return
    }
    setStatus(status);
     ...
     <ImageBackground
      ...
      />
          ...
          <View style={styles.row}>
            ...
            <WordBox wordData={level12[currentIndex]} />
          </View>
