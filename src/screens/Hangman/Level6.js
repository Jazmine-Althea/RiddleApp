...
import { level6 } from './hangmandata';
...
export default function Level6() {
  ...
  const correctWord = level6[currentIndex].answer;
  ...
  const updateStatus = (cl) => {
    ...
    if (status === 'win' && currentIndex === level6.length-1){
      setStatus('completed')
      return
    }
    setStatus(status);
     ...
     <ImageBackground
      ...
      />
          ...
          <View style={styles.row}>
            ...
            <WordBox wordData={level6[currentIndex]} />
          </View>
