...
import { level11 } from './hangmandata';
...
export default function Level11() {
  ...
  const correctWord = level11[currentIndex].answer;
  ...
  const updateStatus = (cl) => {
    ...
    if (status === 'win' && currentIndex === level11.length-1){
      setStatus('completed')
      return
    }
    setStatus(status);
     ...
     <ImageBackground
      ...
      />
          ...
          <View style={styles.row}>
            ...
            <WordBox wordData={level11[currentIndex]} />
          </View>
