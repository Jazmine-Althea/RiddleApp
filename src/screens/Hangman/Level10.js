...
import { level10} from './hangmandata';
...
export default function Level10() {
  ...
  const correctWord = level10[currentIndex].answer;
  ...
  const updateStatus = (cl) => {
    ...
    if (status === 'win' && currentIndex === level10.length-1){
      setStatus('completed')
      return
    }
    setStatus(status);
     ...
     <ImageBackground
      ...
      />
          ...
          <View style={styles.row}>
            ...
            <WordBox wordData={level10[currentIndex]} />
          </View>
