...
import { level1 } from './hangmandata';
import InputBox from './InputBox';
import Keyboard from './Keyboard';
import StatusPopUp from './StatusPopUp';


export default function HangmanLevel() {
  ...
  const correctWord = level1[currentIndex].answer;
  ...
  const updateStatus = (cl) => {
    ...
    if (status === 'win' && currentIndex === level1.length-1){
      setStatus('completed')
      return
    }
    setStatus(status);
     ...
     <ImageBackground
      ...
      />
          ...
          <View style={styles.row}>
            ...
            <WordBox wordData={level1[currentIndex]} />
          </View>
